...
# Helper functions (wrapper for JSON mode compatibility)
...
print_check_wrapper() {
    ...
    if [ "$JSON_OUTPUT" = false ]; then
        print_check "$status" "$message" "$detail"
    fi
}

# 1. ENVIRONMENT DETECTION
check_environment() {
    print_section_wrapper "Environment Detection"

    # Check if in container
    if [ -f /.dockerenv ]; then
        RESULTS["environment"]="container"
        print_check_wrapper "INFO" "Running inside Docker container"

        # Check for Kubernetes
        if [ -n "$KUBERNETES_SERVICE_HOST" ]; then
            RESULTS["orchestrator"]="kubernetes"
            print_check_wrapper "INFO" "Kubernetes environment detected"
        fi
    else
        RESULTS["environment"]="host"
        print_check_wrapper "INFO" "Running on host system"

        # Check for WSL
        if grep -q Microsoft /proc/version 2>/dev/null; then
            RESULTS["platform"]="wsl"
            print_check_wrapper "INFO" "WSL environment detected"
        elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
            RESULTS["platform"]="linux"
            print_check_wrapper "INFO" "Linux system detected"
        elif [[ "$OSTYPE" == "darwin"* ]]; then
            RESULTS["platform"]="macos"
            print_check_wrapper "INFO" "macOS system detected"
        fi
    fi

    # Check architecture
    RESULTS["architecture"]=$(uname -m)
    print_check_wrapper "INFO" "Architecture: $(uname -m)"
}

# 2. KERBEROS CONFIGURATION
check_krb5_config() {
    print_section_wrapper "Kerberos Configuration"

    # Find krb5.conf
    local krb5_conf=""
    if [ -n "$KRB5_CONFIG" ]; then
        krb5_conf="$KRB5_CONFIG"
        print_check_wrapper "INFO" "Using KRB5_CONFIG: $krb5_conf"
    elif [ -f /etc/krb5.conf ]; then
        krb5_conf="/etc/krb5.conf"
        print_check_wrapper "INFO" "Using system krb5.conf: $krb5_conf"
    else
        print_check_wrapper "WARN" "No krb5.conf found"
    ...
    if [ -f "$krb5_conf" ]; then
        ...
        if [ -n "$default_realm" ]; then
            ...
            print_check_wrapper "PASS" "Default realm: $default_realm"
        else
            print_check_wrapper "WARN" "No default realm configured"
        fi

        # Check DNS settings
        if grep -q 'dns_lookup_kdc = true' "$krb5_conf" 2>/dev/null; then
            print_check_wrapper "PASS" "DNS lookup for KDC enabled"
            RESULTS["dns_lookup_kdc"]="true"
        else
            print_check_wrapper "INFO" "DNS lookup for KDC disabled (using static config)"
        ...
        if [ -n "$ticket_lifetime" ]; then
            print_check_wrapper "INFO" "Ticket lifetime: $ticket_lifetime"
        ...
        print_check_wrapper "FAIL" "Cannot read $krb5_conf"
...
check_ticket_cache() {
    print_section_wrapper "Kerberos Ticket Cache"
    ...
    if [ -n "$KRB5CCNAME" ]; then
        ...
        print_check_wrapper "INFO" "KRB5CCNAME is set: $KRB5CCNAME"
    fi
    ...
    print_check_wrapper "INFO" "Scanning for ticket caches..."
    local found_count=0
    ...
    if [ ${#all_caches[@]} -eq 0 ]; then
        print_check_wrapper "FAIL" "No ticket cache found"
        print_check_wrapper "INFO" "Set KRB5CCNAME or run: kinit"
        RESULTS["ticket_cache"]="missing"
        RESULTS["ticket_cache_count"]="0"
        return 1
    elif [ ${#all_caches[@]} -eq 1 ]; then
        print_check_wrapper "PASS" "Found 1 ticket cache: ${all_caches[0]}"
        RESULTS["ticket_cache"]="${all_caches[0]}"
        RESULTS["ticket_cache_count"]="1"
    else
        print_check_wrapper "INFO" "Found ${#all_caches[@]} ticket caches:"
        for cache in "${all_caches[@]}"; do
            if [ "$cache" = "$primary_cache" ]; then
                print_check_wrapper "INFO" "  [PRIMARY] $cache"
            else
                print_check_wrapper "INFO" "  $cache"
            fi
        done
        RESULTS["ticket_cache"]="$primary_cache"
        RESULTS["ticket_cache_count"]="${#all_caches[@]}"
        print_check_wrapper "WARN" "Multiple caches found - using: $primary_cache"
    fi

    # Check ticket validity
    if ! command -v klist >/dev/null 2>&1; then
        print_check_wrapper "WARN" "klist not available - cannot check tickets"
    ...
    print_check_wrapper "INFO" "Examining ticket cache: $ccache"

    if klist -s 2>/dev/null; then
        print_check_wrapper "PASS" "Valid Kerberos ticket found"
        ...
        if [ -n "$principal" ]; then
            ...
            print_check_wrapper "INFO" "Principal: $principal"
        fi

        if [ -n "$expires" ]; then
            print_check_wrapper "INFO" "Expires: $expires"
            ...
            if command -v date >/dev/null 2>&1; then
                ...
                if [ -n "$exp_time" ] && [ "$exp_time" -gt 0 ]; then
                    ...
                    if [ $diff -lt 3600 ] && [ $diff -gt 0 ]; then
                        print_check_wrapper "WARN" "Ticket expires in less than 1 hour!"
                    elif [ $diff -le 0 ]; then
                        print_check_wrapper "FAIL" "Ticket has expired!"
                        RESULTS["ticket_valid"]="expired"
                    fi
                else
                    print_check_wrapper "WARN" "Could not parse expiration time - skipping expiry check"
                fi
            fi
        fi
        ...
        if [ "$VERBOSE_MODE" = true ]; then
            ...
            if klist -f 2>/dev/null | grep -q 'F'; then
                print_check_wrapper "PASS" "Ticket is forwardable"
                RESULTS["ticket_forwardable"]="true"
            else
                print_check_wrapper "WARN" "Ticket is NOT forwardable (may cause delegation issues)"
                RESULTS["ticket_forwardable"]="false"
            fi
        fi
    else
        print_check_wrapper "FAIL" "No valid Kerberos ticket"
        ...
        if echo "$klist_error" | grep -q "No credentials cache found"; then
            print_check_wrapper "INFO" "Run: kinit <username>@<REALM>"
        elif echo "$klist_error" | grep -q "Ticket expired"; then
            print_check_wrapper "INFO" "Ticket expired - run: kinit"
        fi
...
check_network_dns() {
    ...
    print_section_wrapper "Network and DNS Configuration"
    ...
    if [ -n "${RESULTS[default_realm]}" ]; then
        ...
        if command -v nslookup >/dev/null 2>&1; then
            print_check_wrapper "INFO" "Checking DNS for realm: $realm"
            ...
            if nslookup -type=SRV "$srv_record" >/dev/null 2>&1; then
                print_check_wrapper "PASS" "KDC SRV records found for $realm"
                RESULTS["kdc_dns"]="true"
            else
                print_check_wrapper "WARN" "No KDC SRV records found (may use static config)"
                RESULTS["kdc_dns"]="false"
            fi
        elif command -v dig >/dev/null 2>&1; then
            local srv_record="_kerberos._tcp.$realm"
            if dig +short SRV "$srv_record" | grep -q .; then
                print_check_wrapper "PASS" "KDC SRV records found"
                RESULTS["kdc_dns"]="true"
            else
                print_check_wrapper "WARN" "No KDC SRV records found"
                RESULTS["kdc_dns"]="false"
            fi
        else
            print_check_wrapper "INFO" "DNS tools not available"
        fi
    fi

    # Check time synchronization (critical for Kerberos)
    print_check_wrapper "INFO" "Checking time synchronization..."
    ...
    if command -v timedatectl >/dev/null 2>&1; then
        ...
        if echo "$timedatectl_output" | grep -qi "System clock synchronized: yes"; then
            print_check_wrapper "PASS" "Time synchronized (system clock)"
        ...
        # Check for NTP service active (fallback for non-WSL systems)
            ...
            print_check_wrapper "PASS" "Time synchronized via NTP"
            RESULTS["time_sync"]="true"
        # WSL2 shows "n/a" for NTP service, but clock can still be synced
        elif echo "$timedatectl_output" | grep -qi "NTP service: n/a"; then
            print_check_wrapper "INFO" "NTP service n/a (WSL2) - assuming host time sync"
            RESULTS["time_sync"]="wsl_assumed"
        else
            print_check_wrapper "WARN" "Time not synchronized (Kerberos requires <5min skew)"
        ...
        if ntpstat >/dev/null 2>&1; then
            ...
            print_check_wrapper "WARN" "Time not synchronized"
            RESULTS["time_sync"]="false"
        fi
    else
        # Basic time check
        print_check_wrapper "INFO" "Current time: $(date)"
    fi
}
...
check_sql_server() {
    ...
    print_section_wrapper "SQL Server Authentication Test"

    print_check_wrapper "INFO" "Target: $SQL_SERVER / $SQL_DATABASE"
    ...
    if [ -z "$sqlcmd" ]; then
        print_check_wrapper "WARN" "sqlcmd not found - skipping SQL test"
        print_check_wrapper "INFO" "Install: apt-get install mssql-tools18"
        RESULTS["sql_tools"]="missing"
        return 1
    fi

    RESULTS["sql_tools"]="available"
    print_check_wrapper "PASS" "Found sqlcmd: $sqlcmd"

    # Check network connectivity
    print_check_wrapper "INFO" "Testing network connectivity..."
    if command -v nc >/dev/null 2>&1; then
        if timeout 3 nc -zv "$SQL_SERVER" 1433 >/dev/null 2>&1; then
            print_check_wrapper "PASS" "Port 1433 is reachable"
            RESULTS["sql_network"]="true"
        else
            print_check_wrapper "FAIL" "Cannot reach port 1433"
    ...
    if command -v ldapsearch >/dev/null 2>&1 && [ -n "${RESULTS[principal]}" ]; then
        print_check_wrapper "INFO" "Checking SQL Server SPNs..."

        local spn_check="MSSQLSvc/${SQL_SERVER}:1433"
        # This would need proper LDAP configuration
        print_check_wrapper "INFO" "Expected SPN: $spn_check"
    fi

    # Test SQL connection
    print_check_wrapper "INFO" "Testing SQL Server authentication..."
    ...
    if [ $result -eq 0 ]; then
        print_check_wrapper "PASS" "SQL Server connection successful!"
        RESULTS["sql_auth"]="success"

        # Check authentication method
        if echo "$output" | grep -q "KERBEROS"; then
            print_check_wrapper "PASS" "Using KERBEROS authentication"
            RESULTS["sql_auth_method"]="KERBEROS"
        elif echo "$output" | grep -q "NTLM"; then
            print_check_wrapper "WARN" "Using NTLM authentication (not Kerberos)"
            RESULTS["sql_auth_method"]="NTLM"
        else
            print_check_wrapper "INFO" "Authentication method: $(echo "$output" | awk '{print $1}')"
        fi
        ...
        print_check_wrapper "FAIL" "SQL Server connection failed"
        RESULTS["sql_auth"]="failed"

        # Analyze error
        if echo "$output" | grep -q "Login timeout"; then
            print_check_wrapper "INFO" "Timeout - check network/firewall"
            RESULTS["sql_error"]="timeout"
        elif echo "$output" | grep -q "Login failed"; then
            print_check_wrapper "INFO" "Authentication failed - check permissions"
            RESULTS["sql_error"]="auth_failed"
        elif echo "$output" | grep -q "Cannot authenticate using Kerberos"; then
            print_check_wrapper "INFO" "Kerberos auth failed - check SPNs"
...
check_common_issues() {
    ...
    print_section_wrapper "Common Issues Check"

    # Check keytab if exists
    if [ -n "$KRB5_KTNAME" ] || [ -f "/etc/krb5.keytab" ]; then
        local keytab="${KRB5_KTNAME:-/etc/krb5.keytab}"
        if [ -f "$keytab" ]; then
            print_check_wrapper "INFO" "Keytab found: $keytab"

            if command -v klist >/dev/null 2>&1; then
                local keytab_entries=$(klist -k "$keytab" 2>/dev/null | grep -c '@' || echo "0")
                if [ "$keytab_entries" -gt 0 ]; then
                    print_check_wrapper "PASS" "Keytab contains $keytab_entries entries"
                else
                    print_check_wrapper "WARN" "Keytab appears empty"
                fi
            fi
        fi
    fi
    ...
    for var in "${important_vars[@]}"; do
        print_env_var "$var" "${!var}"
    done

    # Check for delegation
    if [ "${RESULTS[ticket_forwardable]}" = "false" ]; then
        print_check_wrapper "WARN" "Ticket not forwardable - may cause issues with delegation"
        print_check_wrapper "INFO" "Fix: kinit -f <username>"
    fi
    ...
    if [ -n "${RESULTS[ticket_cache]}" ]; then
        ...
        if [ -f "$cache_file" ]; then
            ...
            if [ -n "$perms" ]; then
                if [ "$perms" != "600" ]; then
                    print_check_wrapper "WARN" "Ticket cache permissions: $perms (should be 600)"
                fi
            fi
        fi
    fi
...
EOF
    ...
    print_header_wrapper "DIAGNOSTIC SUMMARY"
    ...
    if [ "${RESULTS[ticket_valid]}" != "true" ]; then
        ...
        if [ -n "${NO_COLOR}" ] || [ ! -t 1 ]; then
            echo "[FAIL] No valid Kerberos ticket"
        else
    ...
    if [ -n "${RESULTS[sql_auth]}" ]; then
        if [ "${RESULTS[sql_auth]}" = "success" ]; then
            if [ "${RESULTS[sql_auth_method]}" = "KERBEROS" ]; then
                if [ -n "${NO_COLOR}" ] || [ ! -t 1 ]; then
                    echo "[PASS] SQL Server Kerberos auth working"
                else
                    echo -e "${GREEN}✓ SQL Server Kerberos auth working${NC}"
                fi
            else
                if [ -n "${NO_COLOR}" ] || [ ! -t 1 ]; then
                    echo "[WARN] SQL Server using ${RESULTS[sql_auth_method]} (not Kerberos)"
                else
            ...
            if [ -n "${NO_COLOR}" ] || [ ! -t 1 ]; then
                echo "[FAIL] SQL Server authentication failed"
            else
...
main() {
    if [ "$JSON_OUTPUT" = false ]; then
        print_header_wrapper "KERBEROS AUTHENTICATION DIAGNOSTICS v${VERSION}"
    fi
